...
import { getRequests, removeFromRequestList, updateRequestItem, startDirectDownload, startProwlarrAutoDownload } from '@/services/api';
import { useToast } from '@/components/ToastProvider';
import type { RequestItem, ItemStatus } from '@/types';
...
const RequestsPage: React.FC = () => {
  ...
  const load = () => {
    ...
    getRequests()
      .then(setItems)
      .catch(() => toast('Failed to load request list', 'error'))
  ...
  const handleRemove = async (id: number, title: string) => {
    try {
      await removeFromRequestList(id);
  ...
  const handleStatusChange = async (id: number, status: ItemStatus) => {
    try {
      const updated = await updateRequestItem(id, { status });
  ...
  const handleDirectDownload = async (item: RequestItem) => {
  ...
  const handleProwlarrDownload = async (item: RequestItem) => {
    if (item.status === 'downloading' || processingIds.has(item.id)) {
      return;
    }
  ...
  return (
    <>
      <div className="page-header">
        <h1>📝 Request List</h1>
        <p>{items.length} item{items.length !== 1 ? 's' : ''} requested</p>
      </div>
      ...
      <div className="page-body">
        ...
        {!loading && filtered.length === 0 && (
          <div className="empty-state">
            ...
            <p>Search for books, comics, or manga and add what cannot be found automatically</p>
          </div>
        )}
